...
def get_data(dataset_name, different_new_nodes_between_val_and_test=False, randomize_features=False):
  ...
  if dataset_name == "steam_2017_new":
    val_time, test_time = list(np.quantile(graph_df.ts, [0.31, 0.46]))
  ...
  """ if dataset_name == "steam_2017_new":
    # For steam_2017_new, we use different time splits
    train_mask = np.logical_and(timestamps <= val_time, np.logical_and(timestamps > test_time, observed_edges_mask)) """
...
def get_data_balanced(dataset_name, different_new_nodes_between_val_and_test=False, randomize_features=False):
  ### Load data and train val test split
  graph_df = pd.read_csv('./data/ml_{}.csv'.format(dataset_name))
  edge_features = np.load('./data/ml_{}.npy'.format(dataset_name))
  node_features = np.load('./data/ml_{}_node.npy'.format(dataset_name)) 
