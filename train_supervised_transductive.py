...
parser = argparse.ArgumentParser('TGN transductive training')
...
node_features, edge_features, full_data, train_data, val_data, test_data, new_node_val_data, new_node_test_data = get_data(DATA, randomize_features=args.randomize_features)
...
for i in range(args.n_runs):
  ...
  # Initialize Model
  tgn = TGN(neighbor_finder=ngh_finder, node_features=node_features, edge_features=edge_features, device=device,
            ...
            aggregator_type=args.aggregator, memory_updater_type=args.memory_updater, n_neighbors=NUM_NEIGHBORS,
  ...
  for epoch in range(NUM_EPOCH):
    ...
    for k in range(0, num_batch, args.backprop_every):
      ...
      for j in range(args.backprop_every):
        ...
        sources_batch, destinations_batch = full_data.sources[start_idx:end_idx], full_data.destinations[start_idx:end_idx]
    ...
    val_ap, val_auc = eval_edge_prediction(model=tgn, negative_edge_sampler=val_rand_sampler, data=val_data, n_neighbors=NUM_NEIGHBORS)
    if USE_MEMORY:
  ...
  # Training has finished, we have loaded the best model, and we want to backup its current memory (which has seen validation edges) so that it can also be used when testing on unseen nodes
  if USE_MEMORY:
    val_memory_backup = tgn.memory.backup_memory()
