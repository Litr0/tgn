...
for i in range(args.n_runs):
  ...
  for epoch in range(args.n_epoch):
    ...
    val_auc = eval_abuse_prediction(tgn, decoder_head, val_data, full_data.edge_idxs, BATCH_SIZE,
  ...
  if args.use_validation:
    if early_stopper.early_stop_check(val_auc):
      ...
      torch.save(decoder_head.state_dict(), get_checkpoint_path(epoch))

  if args.use_validation:
    logger.info(f'Loading the best model at epoch {early_stopper.best_epoch}')
    best_model_path = get_checkpoint_path(early_stopper.best_epoch)
    decoder_head.load_state_dict(torch.load(best_model_path))
    logger.info(f'Loaded the best model at epoch {early_stopper.best_epoch} for inference')
    decoder_head.eval()

    test_auc = eval_abuse_prediction(tgn, decoder_head, test_data, full_data.edge_idxs, BATCH_SIZE,
                                     n_neighbors=NUM_NEIGHBORS)
  else:
